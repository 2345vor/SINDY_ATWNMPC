...
| 参数 | 类型 | 默认值 | 描述 |
|------|------|--------|------|
| `--model` | int (1-3) | 1 | 模型类别 (1: 基础模型18参数, 2: 分离模型21参数, 3: 简化模型16参数) |
| `--trajectory` | int (1-3) | 1 | 跟踪曲线 (1: 椭圆, 2: 正弦直线, 3: 双正弦) |
...
1. **权重参数调整**：对于不同的轨迹和环境条件，可能需要调整权重参数以获得最佳性能
2. **约束边界设置**：根据实际系统的物理限制调整约束边界
3. **噪声参数配置**：根据环境噪声特性调整噪声参数，提高鲁棒性
4. **计算性能优化**：对于实时应用，可以调整预测步长和最大迭代次数以平衡精度和计算速度
